(ns saber.nrepl
  ...
  (:require
   ["net" :as node-net]
   ["path" :as path]
   [saber.bencode :refer [encode decode-all]]
   [saber.when-contexts :as when-contexts]
   [saber.repl-utils :as repl-utils :refer [the-sci-ns]]
   [saber.sci :as jsci]
   [saber.utils :refer [info warn error cljify]]
   [promesa.core :as p]
   [sci.core :as sci]
...
(defn server-running? []
  (when-contexts/context ::when-contexts/saber.isNReplServerRunning))
...
(defn- start-server'+
  ...
  (let [opts (cljify js-or-clj-opts)
    ...
    (p/create
     (fn [resolve _reject]
       (let [nrepl-host-address "127.0.0.1"] ;; Todo: From config
         (if (< 0 (node-net/isIP nrepl-host-address))
           (let [server (node-net/createServer
             ...
             (p/do
               (when-contexts/set-context! ::when-contexts/saber.isNReplServerRunning true))
             (.listen server
                      port
                      nrepl-host-address
                      (fn []
                        (let [addr (-> server (.address))
                          ...
                          #_(-> (vscode/workspace.fs.writeFile
             ...
             (.on server "error"
                  (fn [e]
                    (when-contexts/set-context! ::when-contexts/saber.isNReplServerRunning false)
                    (.close server)
                    (error "Problems with the nREPL server connection:" e))))
           (error "Invalid host address, check setting `saber.nreplHostAddress`")))))))
...
(comment
  (-> (start-server+ {:root-path "/Users/selah/Documents/code/saber/" #_"/hello-joyride"})
